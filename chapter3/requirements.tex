...
\begin{center}
  \begin{userrequirement}[Browser compatibility]
    ...
    \requirementprio{High}
    \requirementphase{Initial phase}
    \requirementdesc{It shall be compatible with latest versions of all popular devices: \ida{IE} 7+, \idx{Firefox}, \idx{Opera}, \idx{Safari} and \idx{Google Chrome}.
    The only exception shall be \ida{IE} 6, for practical reasons.}
  \end{userrequirement}
\end{center}
...
\begin{center}
  \begin{userrequirement}[Integrate the \idx{IPTVplus} interface]
    ...
    \requirementdesc{The \idx{IPTVplus} interface (see Figure~\vref{fig:iptvplus}) must be implemented as a sidebar inside the \ida{PNAI} interface.
...
\begin{center}
  \begin{userrequirement}[Resize/collapse sidebar]
    ...
    Therefore, the \idx{IPTVplus} interface (see Figure~\vref{fig:iptvplus}) must be implemented as a sidebar inside the \ida{PNAI} interface.}
  \end{userrequirement}
\end{center}
