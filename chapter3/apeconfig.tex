\section{APE Server\index{APE server} Installation and Configuration}
\label{sec:apeconfig}

In this section the installation and configuration of the
\idx{APE server} are defined step by step.
...
\idx{Application Server} by typing Listing~\ref{apeinstallation} from the same
...
After that, the APE server daemon (\idc{aped}) is automatically started with
the default configuration \cite{ApeSetup}. It can be checked by visiting the
url \url{webportal.imusu.mobile.dtrd.de:6969}.

\subsection{Configure \idc{BIND}}

The \idx{IMS core} is the machine that provides the \idc{DNS} service through
\idc{BIND}, and that service needs to be configured to allow the
\idx{APE server} to use a lot of different dynamic subdomains like
\verb|1.ape.webportal|, \verb|2.ape.webportal|, \verb|567.ape.webportal|,
etc.

This is how the \idx{APE server} works by default, and it appears that there
is no way to configure the \idx{APE server} for using only one domain
\cite{ApeConfig}.

So, in the file \verb|/etc/bind/imusu.dnszone| located in the \idx{IMS core}
...
To apply the changes, we have to restart \idc{BIND} using the command in
