\section{JavaScript Framework: \idx{MooTools}}
\label{sec:mootools}

Before explaining why \idx{MooTools} has been chosen for this application,
another important question has to be resolved:

\subsection{Why Use a \idx{JavaScript} Framework?}
...
\idx{Internet Explorer} and the rest of the browsers.
  \item Because we want to facilitate the development, since usually these
frameworks cover several holes in the \idx{JavaScript} specification that
...
\idx{jQuery}\footnote{jQuery is available on:
  \url{http://jquery.com/}},
\idx{Prototype}\footnote{Prototype is available on:
  \url{http://www.prototypejs.org/}},
\idx{Dojo}\footnote{Dojo is available on:
  \url{http://www.dojotoolkit.org/}},
\idx{YUI}\footnote{YUI is available on:
  \url{http://developer.yahoo.com/yui/}},
\idx{GWT}\footnote{GWT is available on:
  \url{http://code.google.com/webtoolkit/}},
\idx{Ext JS}\footnote{Ext JS is available on:
...
consideration, \idx{MooTools} \cite{MooTools} was considered the
...
offers. Particularly, it is more optimized in this aspect than \idx{Prototype},
\idx{YUI} or \idx{Dojo}, but it is also slightly more compact than
\idx{jQuery}.
...
\idx{Internet Explorer} 6+, \idx{Firefox} 2+, \idx{Opera} 9+, \idx{Safari} 2+
(and other \idx{Webkit}-based browsers, like \idx{Chrome}).
  \item[Functional] It offers all the functionality required for the first phase
of the project: \idx{drag\&drop}, resize, animations, etc.

It also offers other functionality like \idx{AJAX} support, \idc{Hash}
handling or \idc{Cookie} handling, that ease the development in different
browsers.
  \item[Object-Oriented] By adding \emph{Classes} to \idx{JavaScript}, an
...
server and in the client. Moreover, the inherited code for \idx{ScaleNet}
already used \idx{JavaScript} objects.
  \item[Extensive] It also has a repository for official plugins called 
\idx{MooTools More} (with similar code quality and documentation to the
\idx{MooTools Core}) and other third-party plugins can be found in the web.
...
application. Frameworks like \idx{jQuery} are more focused in reducing the 
lines of code that in encouraging robust coding. \idx{MooTools} also helps
...
\idx{JavaScript}. Another important point of this framework is that it is
based on prototype extensions (mainly \idc{DOM} extensions), so the syntax is
very Object-Oriented and the code seems very clean.
  \item[Used by the \idx{APE server}] (See Section~\ref{sec:ape}) So if we use that component, it will be
very straightforward to write extensions in \idx{JavaScript} also in the
