...
Figure~\ref{fig:html5-structure} introduces the new \idc{article}, \idc{aside}, \idc{header}, \idc{footer} and \idc{nav} elements and compares it with the traditional \ida{HTML} 4 approach.
...
\begin{figure}[htbp]
  ...
  \caption{Structure of \idx{HTML} 5 compared to \idx{HTML} 4}
...
If \idx{JavaScript} code is too inefficient for the task, there is a \ida{HTML} 5 extension to run compile native code (written in C/C++).
...
For example, \idc{border-radius} was called \ida{-webkit-border-radius} in \idx{Webkit} browsers, \ida{-moz-border-radius} in \idx{Firefox} and \idx{-o-border-radius} in \idx{Opera}.
Later, when the standard was more settle, they all change it to \ida{border-radius}.
